...
class MotionDetectionEngine(
    ...
    private var faceDetectionWarningLogged = false
    ...
    suspend fun processImageProxy(imageProxy: ImageProxy) {
        if (detectorMode == MotionDetectionMode.FACE_DETECTION) {
            if (!faceDetectionWarningLogged) {
                Timber.w("Face detection is unavailable in this build (googlefree) — motion detection disabled while in FACE_DETECTION mode")
                faceDetectionWarningLogged = true
            }
        }
    }
